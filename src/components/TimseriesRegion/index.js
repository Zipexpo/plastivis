...
export default function({region,colorBySims,onChangeRegion}) {
    const {getTimeseriesByRegion,getValue,setValue,getTimeseriesNetworkByRegion} = useData();
    ...
    useEffect(()=>{
        if (areaChooses.length) {
            ...
            Promise.all(
                querys.map(q=> {
                    if (q.network){
                        return getTimeseriesNetworkByRegion(q.query, {
                        ...
                        return getTimeseriesByRegion(q.query, {
                            signal: controllerS.signal
                        }).then((data) => {
                            debugger
