...
export default function(){
    const {getdata3D,getdata3Dconnect,get3DdataRegion,getAreaConnection,setValue,getValue,getApi,colorBySims} = useData();
    const [neuronId,setNeuronId] = useState(1);
    const [region,setRegion] = useState({'area_8':true,'area_30':true,'area_34':true});
    const [netData,setNetData] = useState({});
    const [_regionData,set_regionData] = useState(emptyObject);
    ...
    const areaConnectionList = getValue('areaConnectionList')??emptyObject;
    ...
    useEffect(()=>{
        ...
        setNetData(sims);
        set_regionData(regionData)
    },[data3Dconnect,regionData])
            ...
            <Grid container>
                <Grid xs={12} sx={{height: 500}}>
                    <Viz3D data={data3D}
                           ...
                           subdataValue={_regionData}
                           netData={netData}
                           netBySub={areaConnectionList}
                           ...
                           areaSeparation={setting3D.areaSeparation}
                    />
                </Grid>
                <Grid xs={12}>
                    <PlayPanel
                ...
                <Grid xs={12} sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs value={viewMode} onChange={handleTabChange} aria-label="basic tabs example">
                        ...
                        <Tab label="Neuron Timeline" disabled {...a11yProps(1)} />
                        ...
                        <Tab label="Parallel Coordinates" {...a11yProps(4)} />
                    </Tabs>
                </Grid>
