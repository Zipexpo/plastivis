...
import {
    csv, blob, max as d3max, min as d3min, mean as d3mean, scaleOrdinal, range as d3range
} from "d3";
...
const Provider = ({children, name}) => {
    ...
    const colorBySims = useMemo(()=>scaleOrdinal().range(["#1f77b4","#ff7f0e","#2ca02c","#9467bd","#8c564b","#e377c2","#7f7f7f","#bcbd22","#17becf"]).domain(state.simsList.value),[state.simsList.value]);
    useEffect(() => {
        // init function if needed
        dispatch({type: 'LOADING_CHANGED', path: 'monitorData', isLoading: 1});
        let delta = 100/state.metricList.value.length;
        let count = delta;
        Promise.all(state.metricList.value.map(d=>blob(`./data/${d}.json`).then(d=>d.arrayBuffer()).then(d=> {
            dispatch({type: 'LOADING_CHANGED', path: 'monitorData', isLoading: count});
            count+=delta;
            return JSON.parse(inflate(d, {to: 'string'}))
        }))).then(d=>{
            const monitorData = {};
            state.metricList.value.forEach((m,i)=>monitorData[m] = d[i])
            dispatch({type: 'VALUE_CHANGE', path:'monitorData', value:monitorData, isLoading: false});
        })

        dispatch({type: 'LOADING_CHANGED', path: 'netData', isLoading: 1});
        let deltaNet = 25;
        let countNet = deltaNet;
        Promise.all(state.simsList.value.map(d=>blob(`./data/${d}_connectionList.json`).then(d=>d.arrayBuffer()).then(d=> {
            dispatch({type: 'LOADING_CHANGED', path: 'netData', isLoading: countNet});
            countNet+=deltaNet;
            return JSON.parse(inflate(d, {to: 'string'}))
        }))).then(d=>{
            const netData = {};
            state.simsList.value.forEach((m,i)=>netData[m] = d[i])
            dispatch({type: 'VALUE_CHANGE', path:'netData', value:netData, isLoading: false});
        })

        dispatch({type: 'LOADING_CHANGED', path: 'connectionTimeline', isLoading: 1});
        blob(`./data/connectionTimeline.json`).then(d=>d.arrayBuffer()).then(d=> {
            dispatch({type: 'VALUE_CHANGE', path:'connectionTimeline', value:JSON.parse(inflate(d, {to: 'string'})), isLoading: false});
        })


        dispatch({type: 'LOADING_CHANGED', path: 'PCAdata', isLoading: 1});
        blob(`./data/PCA.json`).then(d=>d.arrayBuffer()).then(d=> {
            dispatch({type: 'VALUE_CHANGE', path:'PCAdata', value:JSON.parse(inflate(d, {to: 'string'})), isLoading: false});
        });

        dispatch({type: 'LOADING_CHANGED', path: 'stimulusaverage', isLoading: 1});
        let countstimu = 50;
        Promise.all([stimulus_nonstim_created,stimulus_nonstim_deleted].map(d=>csv(d).then(
            d=>{
                dispatch({type: 'LOADING_CHANGED', path: 'stimulusaverage', isLoading: countstimu});
                countstimu+=50;
                return d;
            }
        )))
    ...
    const getTimeseriesBrain = useCallback(()=>{
         return blob(`./data/summaryTimeline.json`).then(d=>d.arrayBuffer()).then(d=> {


            return JSON.parse(ungzip(d, {to: 'string'}));
         })
    },[])
    ...
    const getdata3Dconnect = useCallback((savedList)=>{
        ...
        if (savedList){
            ...
            new Promise((resolve, reject) => {
                const data={"createdin":{},"createdout":{},"deletedin":{},"deletedout":{}};
                const timestep = Math.round(query.timestep/10000)-1;
                query.sims.forEach(k=>{
                    data.createdin[k] = state.netData.value[k][0][timestep]??[];
                    data.createdout[k] = state.netData.value[k][1][timestep]??[];
                    data.deletedin[k] = state.netData.value[k][2][timestep]??[];
                    data.deletedout[k] = state.netData.value[k][3][timestep]??[];
                })
                if (signal?.aborted){
    ...
    },[state.timestep.value,state.sims.value,state.netData.value])
    ...
    const get3DdataRegion = useCallback(()=>{
        ...
        if (inlist!==-1){
            ...
            new Promise((resolve, reject) => {
                ...
                query.sims.forEach(k=>{
                    regionmap[k] = {};
                    state.monitorData.value[query.metric][k].forEach(d=> {
                        if (d.timestep===Math.round(query.timestep/100)*100)
                            regionmap[k][d.area] = d.value
                    })
                })
                if (signal?.aborted){
                    reject(new DOMException("Aborted", "AbortError"));
    ...
    },[state.timestep.value,state.sims.value,state.setting3D.value.metric,state.monitorData.value]);
    const getTimeseriesNetworkByRegion = useCallback((query,signal)=>{
        return new Promise((resolve, reject) => {
            const regionmap=[];
            [0,1,2,3].forEach(i=>regionmap[i] = {})
            query.sims.forEach(k=>{
                state.connectionTimeline.value[query.region].forEach((d,i)=> {
                    regionmap[i][k] = d[k]
                });
            })
            if (signal?.aborted){
        ...
        //     reject(true);
        // })
    },[state.connectionTimeline.value])
    const getTimeseriesByRegion = useCallback((query,signal)=>{
            return new Promise((resolve, reject) => {
                debugger
                const regionmap={x:d3range(0,1000000,100),y:[]};
                query.sims.forEach(k=>{
                    regionmap.y[k]=[];
                    state.monitorData.value[query.metric][k].forEach((d,i)=> {
                        if (d.area === query.region) {
                            regionmap.y[k][Math.round(d.timestep/100)] = d.value;
                        }
                    });
                })
                if (signal?.aborted){
